...
import { createUser, loginUser, loginUserWithSSO, signUpWithSSO } from '../services/userService';
...
const useAuth = (authType: 'login' | 'signup') => {
  ...
  /**
   * handleGoogleLogin - Calls loginUserWithSSO to authenticate a user via Google
   * @param googleToken - Google ID token
   */
  const handleGoogleLogin = async (googleToken: string) => {
    try {
      const user = await loginUserWithSSO(googleToken);
      setUser(user);
      navigate('/home');
    } catch (error) {
      setErr((error as Error).message);
  ...
  /**
   * handleGoogleSignup - Calls signUpWithSSO to create a new user via Google
   * @param googleToken - Google ID token
   */
  const handleGoogleSignup = async (googleToken: string) => {
    try {
      const user = await signUpWithSSO(googleToken);
      setUser(user);
      navigate('/home');
    } catch (error) {
      setErr((error as Error).message);
