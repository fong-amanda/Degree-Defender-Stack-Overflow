...
import { updateUserWins } from '../user.service';
...
class GameManager {
  ...
  public async leaveGame(
    ...
    try {
      ...
      const winnableState = gameToLeave.state as WinnableGameState;
      // console.log('Winnable state:', winnableState);
      // console.log('Game state:', gameToLeave.state);
      // console.log('Game state status:', gameToLeave.state.status);

      // Check if the game is over and has winners
      if (winnableState.status === 'OVER' && winnableState.winners?.length) {
        const winner = winnableState.winners[0];
        if (winner) {
          await updateUserWins(winner);
        }
      }
