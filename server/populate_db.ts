...
// Pass URL of your mongoDB instance as first argument(e.g.,
 ...
 */
async function tagCreate(name: string, description: string): Promise<
  DatabaseTag
> {
...
const populate = async () => {
  try {
    ...
    const c1 = await commentCreate(C1_TEXT, 'sana', new Date(
      '2023-12-12T03:30:00',
    ));
    const c2 = await commentCreate(C2_TEXT, 'ihba001', new Date(
      '2023-12-01T15:24:19',
    ));
    const c3 = await commentCreate(C3_TEXT, 'saltyPeter', new Date(
      '2023-12-18T09:24:00',
    ));
    const c4 = await commentCreate(C4_TEXT, 'monkeyABC', new Date(
      '2023-12-20T03:24:42',
    ));
    const c5 = await commentCreate(C5_TEXT, 'hamkalo', new Date(
      '2023-12-23T08:24:00',
    ));
    const c6 = await commentCreate(C6_TEXT, 'azad', new Date(
      '2023-12-22T17:19:00',
    ));
    const c7 = await commentCreate(C7_TEXT, 'hamkalo', new Date(
      '2023-12-22T21:17:53',
    ));
    const c8 = await commentCreate(C8_TEXT, 'alia', new Date(
      '2023-12-19T18:20:59',
    ));
    const c9 = await commentCreate(C9_TEXT, 'ihba001', new Date(
      '2022-02-20T03:00:00',
    ));
    const c10 = await commentCreate(C10_TEXT, 'abhi3241', new Date(
      '2023-02-10T11:24:30',
    ));
    const c11 = await commentCreate(C11_TEXT, 'Joji John', new Date(
      '2023-03-18T01:02:15',
    ));
    const c12 = await commentCreate(C12_TEXT, 'abaya', new Date(
      '2023-04-10T14:28:01',
    ));

    const a1 = await answerCreate(A1_TXT, 'hamkalo', new Date(
      '2023-11-20T03:24:42',
    ), [c1]);
    const a2 = await answerCreate(A2_TXT, 'azad', new Date(
      '2023-11-23T08:24:00',
    ), [c2]);
    const a3 = await answerCreate(A3_TXT, 'abaya', new Date(
      '2023-11-18T09:24:00',
    ), [c3]);
    const a4 = await answerCreate(A4_TXT, 'alia', new Date(
      '2023-11-12T03:30:00',
    ), [c4]);
    const a5 = await answerCreate(A5_TXT, 'sana', new Date(
      '2023-11-01T15:24:19',
    ), [c5]);
    const a6 = await answerCreate(A6_TXT, 'abhi3241', new Date(
      '2023-02-19T18:20:59',
    ), [c6]);
    const a7 = await answerCreate(A7_TXT, 'mackson3332', new Date(
      '2023-02-22T17:19:00',
    ), [c7]);
    const a8 = await answerCreate(A8_TXT, 'ihba001', new Date(
      '2023-03-22T21:17:53',
    ), [c8]);
