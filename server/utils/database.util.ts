...
const populateGroupChat = async (chatID: string): Promise<PopulatedDatabaseGroupChat | null> => {
  const groupChatDoc = await GroupChatModel.findOne({ _id: chatID }).populate<{
    messages: DatabaseMessage[];
  }>([{ path: 'messages', model: MessageModel }]);
  if (!groupChatDoc) {
  ...
  const messagesWithUser: Array<MessageInChat | null> = await Promise.all(
    groupChatDoc.messages.map(async (messageDoc: DatabaseMessage) => {
      if (!messageDoc) return null;
  ...
  // Filters out null values
  const enrichedMessages = messagesWithUser.filter(Boolean);
  const transformedGroupChat: PopulatedDatabaseGroupChat = {
    ...groupChatDoc.toObject(),
    messages: enrichedMessages as MessageInChat[],
  };

  return transformedGroupChat;
};
...
export const populateDocument = async (
  id: string,
  type: 'question' | 'answer' | 'chat' | 'groupchat',
  // 'groupchat',
): Promise<
  | PopulatedDatabaseAnswer
  | PopulatedDatabaseChat
  ...
  try {
    ...
    switch (type) {
      ...
      case 'chat':
        ...
        break;
      case 'groupchat':
        result = await populateGroupChat(id);
        break;
      default:
